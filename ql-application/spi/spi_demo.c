...
//unsigned char ReadAddress[4]= {READ_DATA_ID,0x11,0x01,0x00}; // 
unsigned char ReadAddress[4]= {0};
char rcv_buf[]  = "049.00,018.00,086.00,2.0,10,050.00,050.00,20.00,230.00,45.00,550.68,50.00,650.00,035.00,546553.50,123.11,123456.00,12345678.00,456.12,55.89,789.32,911.00,85.69,14369.00,231129213,2025-02-04 13:12:35,25,278730,901943,18,1,10,160424,148";
...
static void ql_spi_demo_task_pthread(void *ctx)
{
    ...
    unsigned char DIndex = 1;   // day index for the block to be erased
    unsigned char pIndex = 1;  // packet index for the page to be written
    ...
    if(ret_spi == QL_SPI_SUCCESS) 
    {
       ...
       WritePage(DIndex,pIndex,rcv_buf);
    ...
    while (1) 
    {
        ...
        ql_rtos_task_sleep_s(3);
        ReadAddress [0]= READ_DATA_ID;
        ReadAddress [1]= (unsigned char)((PACKET_ADDRESS(DIndex,pIndex)>> 16) & 0xFF); // high byte of address
        ReadAddress [2]= (unsigned char)((PACKET_ADDRESS(DIndex,pIndex)>> 8) & 0xFF); // mid byte of address
        ReadAddress [3]= (unsigned char)(PACKET_ADDRESS(DIndex,pIndex) & 0xFF); // low byte of address
