...
static void ql_spi_demo_task_pthread(void *ctx)
{
    ...
    outlen = 10;//QL_SPI_DMA_IRQ_SIZE; // Adjust size for polling
    inlen = 10;//QL_SPI_DMA_IRQ_SIZE;

    out_mal_data = (unsigned char *)malloc(outlen);
    in_mal_data = (unsigned char *)malloc(inlen);
...
// outdata[0] = 0x9f;
    ...
    while (1) 
    {
        ...
        ql_spi_write_read(QL_CUR_SPI_PORT, indata, outdata,3);

        // Write data using polling
       // ql_spi_write(QL_CUR_SPI_PORT, outdata, 4);

        // Read data using polling
       // ql_spi_read(QL_CUR_SPI_PORT, indata, 4);
