...
int fota_ftp_server_connect(fota_ftp_client_t *fota_ftp_cli_p)
{
    ...
    ql_bind_sim_and_profile(fota_ftp_cli_p->nSim, fota_ftp_cli_p->profile_idx, &sim_cid);
...
void ql_fota_ftp_app_init()
{
    ...
    err = ql_rtos_task_create(&fota_ftp_task, 1024*16, APP_PRIORITY_REALTIME, "fota_ftp_app", fota_ftp_app_thread, NULL, 5);
