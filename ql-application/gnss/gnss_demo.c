    ...
    switch(nmea->type)
    {
        case NMEA_RMC:
            ...
            if(rmc)
            {
				...
				nmea_dbg_log("GPS latitude :%ld\r\n",gps_data->latitude);
				nmea_dbg_log("GPS Longitude :%ld\r\n",gps_data->longitude);
            }
        break;
        case NMEA_GSV:
        ...
        case NMEA_GGA:
            ...
            if(gga != NULL)
            {
				...
				nmea_dbg_log("No of Sattelites Tracked :%d\r\n",gps_data->satellites_num);
				if(gps_data->satellites_num > 5)
				{
					satteliteCount = gps_data->satellites_num;
					// ql_gpio_set_level(GPIO_30 ,LVL_LOW);
					// nmea_dbg_log("led 30 OFF");
					// ql_rtos_task_sleep_ms(50);
					//ql_gpio_set_level(GPIO_30 ,LVL_HIGH);
					nmea_dbg_log("led 30 OFF");
					nmea_dbg_log("GPS locked \r\n");
					// ql_rtos_task_sleep_ms(50); 
					ql_rtos_task_delete(NULL);
