...
static void _gpioint_callback01(void *ctx)
{
    ...
    QL_GPIOINTDEMO_LOG("gpio[%d] int lvl:[%d]", GPIO_2, gpio_lvl);
...
static void ql_gpioint_demo_thread(void *param)
{
    ...
    ql_int_register(GPIO_2, EDGE_TRIGGER, DEBOUNCE_EN, EDGE_FALLING, PULL_DOWN, _gpioint_callback01, NULL);
    ql_int_register(GPIO_1, EDGE_TRIGGER, DEBOUNCE_EN, EDGE_RISING, PULL_DOWN, _gpioint_callback02, NULL);
    ...
    while(1)
    {
        ...
        {
            if( ql_int_disable(GPIO_2) == 0 )
            {
                QL_GPIOINTDEMO_LOG("gpio[%d] int disable, enable gpio[%d] int", GPIO_2, GPIO_1);
                ql_int_enable(GPIO_1);
        ...
        else/* ql_intsel_flg == INT_CB02 */
        {
            ...
            {
                QL_GPIOINTDEMO_LOG("gpio[%d] int disable, enable gpio[%d] int", GPIO_1, GPIO_2);
                ql_int_enable(GPIO_2);
