...
static bool ql_psm_set_cfun(void)
{
    ...
    ret = ql_dev_set_modem_fun(0, 0, QL_PSMDEMO_SIM_ID);
    if(ret != QL_DEV_SUCCESS)
    {
        QL_PSMDEMO_LOG("set QL_DEV_CFUN_MIN, ret: 0x%x", ret);
        return false; 
    }

    ret = ql_dev_set_modem_fun(1, 0, QL_PSMDEMO_SIM_ID);
    if(ret != QL_DEV_SUCCESS)
    {
...
static void ql_psm_demo_thread(void *param)
{
    ...
    if(0 == ql_get_powerup_reason(&source) && QL_PWRUP_PSM_WAKEUP == source)
    {
        ...
        if(QL_SLEEP_SUCCESS == ql_psm_sleep_enable(QL_PSMDEMO_SIM_ID, true, NULL, active_time))
        {
