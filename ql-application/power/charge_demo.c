...
#define QL_SET_USB_CHARGE_PERIOD  0     //获取和设置USB充电周期
...
#if QL_SET_USB_CHARGE_PERIOD
    uint32_t ontime = 0,offtime = 0,detime = 0;
    ql_get_usb_charge_period(&ontime,&offtime); 
    ql_get_usb_detect_max_time(&detime);
	QL_CHARGERDEMO_LOG("ontime:%d, offtime:%d, detime:%d\r\n", ontime, offtime,detime);
       
	ql_set_usb_charge_period(8000,2000);
    ql_set_usb_detect_max_time(2000);
    ql_get_usb_charge_period(&ontime,&offtime);
	QL_CHARGERDEMO_LOG("ontime:%d, offtime:%d, detime:%d\r\n", ontime, offtime,detime);
#endif
