...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_BOOT_FAT
...
int bootVfsOpen(const char *path, int flags, ...)
{
    ...
    QUEC_BOOT_VFS_LOG("bootVfsOpen path/%s",path);
    #endif
    int qret = quec_bootVfsOpen(path);
    if(qret >= 0)
    {
        return qret;
    }
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_BOOT_FAT
    #if QUEC_BOOT_VFS_ECHO 
    QUEC_BOOT_VFS_LOG("bootVfsUnlink path%s",path);
    #endif
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_BOOT_FAT
    #if QUEC_BOOT_VFS_ECHO 
    QUEC_BOOT_VFS_LOG("bootVfsFileSize");
    #endif
#endif    
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE_BOOT_FAT
    #if QUEC_BOOT_VFS_ECHO 
        QUEC_BOOT_VFS_LOG("bootVfsFileRead");
    #endif
#endif
